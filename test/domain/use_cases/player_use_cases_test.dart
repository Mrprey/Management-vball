...
void main() {
  group('PlayerUseCases', () {
    ...
    test(
        'RegisterPlayerUseCase - should register a new player with valid position',
        () async {
      // Act
      final player =
          await registerUseCase.execute('John Doe', 10, PlayerPositions.setter);
    ...
    test(
        'RegisterPlayerUseCase - should register players with all valid positions',
        () async {
      // Test all valid positions
      await registerUseCase.execute(
          'Player1', 1, PlayerPositions.setter); // L - Setter
      await registerUseCase.execute('Player2', 2,
          PlayerPositions.outsideHitter1); // P1 - Outside Hitter 1
      await registerUseCase.execute('Player3', 3,
          PlayerPositions.outsideHitter2); // P2 - Outside Hitter 2
      await registerUseCase.execute(
          'Player4', 4, PlayerPositions.opposite); // O - Opposite
      await registerUseCase.execute('Player5', 5,
          PlayerPositions.middleBlocker1); // M1 - Middle Blocker 1
      await registerUseCase.execute('Player6', 6,
          PlayerPositions.middleBlocker2); // M2 - Middle Blocker 2
      await registerUseCase.execute(
          'Player7', 7, PlayerPositions.libero); // Lib - Libero
    ...
    test('RegisterPlayerUseCase - should throw error for invalid position',
        () async {
      // Act & Assert
      expect(
          () async =>
              await registerUseCase.execute('John Doe', 10, 'InvalidPosition'),
    ...
    test('RegisterPlayerUseCase - should throw error for empty name', () async {
      ...
      expect(
          () async =>
              await registerUseCase.execute('', 10, PlayerPositions.setter),
          throwsA(isA<Exception>()));
    });

    test('RegisterPlayerUseCase - should throw error for invalid number',
        () async {
      // Act & Assert
      expect(
          () async => await registerUseCase.execute(
              'John Doe', 0, PlayerPositions.setter),
          throwsA(isA<Exception>()));
      expect(
          () async => await registerUseCase.execute(
              'John Doe', -5, PlayerPositions.setter),
          throwsA(isA<Exception>()));
